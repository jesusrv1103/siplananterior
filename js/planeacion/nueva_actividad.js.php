document.getElementById("tipo_med").disabled=true;
...
function validar(){
	...
	if(parseFloat($('#ponderacion').val()) > <?php echo 100 - $total; ?> ){alert('Ponderación debe ser menor o igual a '+<?php echo 100 - $total; ?>); return false;}
...
function numero(){
	...
	$actividades = $conexion->query("SELECT no_accion FROM acciones WHERE id_proyecto = ".$_GET['id_proyecto']." and id_componente=".$_GET['id_componente']);
	$conexion->close();
	unset($conexion);
	while($actividad = $actividades->fetch_array()){ ?>
		if (<?php echo (int)$actividad[0]; ?> == parseInt($('#num').val())) {
			return false;
		}
		<?php } ?>
		return true;
		...
		if(validado){
			...
			$.ajax({
				...
				if (msg == "ponderacion") {
					...
					alert('Se ha guardado correctamente la actividad');
