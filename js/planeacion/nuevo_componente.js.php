function agregar_tipo_unidad(idUnidad){
    'use strict';
    ...
    switch(idUnidad){
        ...
        $consultaUnidades = $conexion->query("SELECT id_unidad FROM u_medida_prog01 order by id_unidad ASC");
        $conexion->close();
        unset($conexion);
        while($res_unidades = $consultaUnidades->fetch_array()){
            echo 'case "'.$res_unidades[0].'": '."\n";
            $conexion = $conn -> conectar(1);
            $consultaTiposUnidad = $conexion->query("SELECT id_tipo_unidad,id_tipo,nombre FROM tipo_unidad_prog01 WHERE id_unidad = ".$res_unidades[0]." ORDER BY id_tipo ASC");
              $conexion->close();
              unset($conexion);
              $contador = 1;
              while($resTipo = $consultaTiposUnidad->fetch_array()){ ?>
                  document.getElementById('tipo_unidad_medida').options[<?php echo $contador; ?>] = new Option('<?php echo $resTipo[1]." - ".$resTipo[2]; ?>', '<?php echo $resTipo[0]; ?>');
                  <?php 
                  $contador++; 
              }
              unset($contador);
              unset($resTipo);
              unset($consultaTiposUnidad);
...
function validar_formulario(){
    ...
    if( $('#no_componente').val().length < 1 || isNaN( $('#no_componente').val())){ alert('error en el numero de componente'); return false; }
    console.log($('#u_responsable').val());
    var componentesid = new Array(<?php for($x=0;$x<$totalcomponentes;$x=$x+1){
        echo $numcomponentes[$x].",";
    }  echo "0"; ?>);
    for(x=0;x < componentesid.length;x++){
        if(document.getElementById('no_componente').value == componentesid[x]){
            alert("este n\u00famero de componente ya existe porfavor borre el compoonente o seleccione otro n\u00famero");
    ...
    if($('#ponderacion').val() > <?php echo $ponderacionmax; ?> ){ alert('la ponderación sobrepasa el valor permitido');  return false}
    if( $('#u_medida').val().trim() === '') { alert('Debe seleccionar clasificacion programatica'); return false;}
    if( $('#tipo_unidad_medida').val().trim() === '') { alert('Debe seleccionar clasificacion programatica'); return false;}
    if( $('#cantidad').val().lenght < 1 ){ alert('error en la cantidad'); return false;}
    if( $('#dep_responsable').val().trim() === '') { alert('Debe seleccionar dependencia responsable'); return false;}
    if( $('#u_responsable').val().trim() === '') { alert('Debe seleccionar unidad responsable'); return false;}
...
function guardar(){
    var validado = validar_formulario();
    ...
    if(validado){
        ...
        .done(function( msg ) {
            console.log(msg);
            alert( 'Se ha guardado correctamente el componente' );
...
function carga_unidades_responsables(v){
  'use strict';
  if(v == ''){return false;};
