...
function agregaestrategia(idsubprograma) {
    ...
    switch (idsubprograma) {
        ...
        document.getElementById('estrategia').options[2] = new Option('2.1.2 Promover la cltura de derechos humanos...', '21');
...
function agregafuncion(idfinalidad) {
    ...
    if (idfinalidad) {
     $.ajax({
        ...
        for(var x = 0; x < unidades_responsables.length; x++){
            document.getElementById('funcion').options[x+1] = new Option(unidades_responsables[x][1], unidades_responsables[x][0]);
        }
    }); 
}
    /*
    ...
    for(x=0;x<proyectosid.length;x++){
        if(document.getElementById('no_proyecto').value == proyectosid[x]){
            ...
            document.getElementById('no_proyecto').focus();
            return false;
        }
    }
    ...
    if( $('#objetivo').val().lenght < 8 ){ alert('error en el objetivo( Fin )'); return false;}
...
function guardar(){
    ...
    if($('#prioritario').prop('checked')){ var clasificacion = 1; } else { var clasificacion = 0;}
    if(validado){
