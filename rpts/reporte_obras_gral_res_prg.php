...
$this->SetXY(3,28);
if($_GET['dep']!="subprg" and  $_GET['dep']!="prg" ){
...
if($_GET['subprg']!=""){
	$this->SetFont('Arial','B',10);
$this->SetXY(148,28);
	$resultado_sec = mysql_query("SELECT * FROM subprogramas_poa WHERE id_subprograma_poa = ".$_GET['subprg']);// or die (mysql_error());
$res_sec = mysql_fetch_array($resultado_sec);
$this->MultiCell(25,5,"Subprograma ",0,L,0); 
$this->SetFont('Arial','',10);
$this->SetXY(170,28);
$this->MultiCell(10,5,$res_sec['clave'],0,C,0); 
$this->SetFont('Arial','',10);
$this->SetXY(180,28);
$this->MultiCell(170,5,utf8_decode($res_sec['descripcion']),0,L,0); 
...
$resultado_sec = mysql_query("SELECT * FROM programas_poa WHERE id_programa_poa = ".$_GET['prg']);// or die (mysql_error());
$res_sec = mysql_fetch_array($resultado_sec);
$this->MultiCell(20,5,"Programa ",0,L,0); 
$this->SetFont('Arial','',10);
$this->SetXY(165,28);
$this->MultiCell(15,5,$res_sec['clave'],0,C,0); 
$this->SetFont('Arial','',10);
$this->SetXY(183,28);
$this->MultiCell(170,5,utf8_decode($res_sec['descripcion']),0,L,0); 
...
if ($_GET['ap']=="1"){
$this->MultiCell(125,5,'ESTRUCTURA FINANCIERA 2014 (PESOS) APROBADA',1,'C',0);
}else{
$this->MultiCell(125,5,'ESTRUCTURA FINANCIERA 2014 (PESOS) PROGRAMADA',1,'C',0);	
	}
$this->SetXY(293,35);
...
INNER JOIN poa02_origen ON obras.id_obra = poa02_origen.id_obra
	  		 WHERE  `dependencias`.`acronimo` = '".$_GET['dep']."' AND obras.subprograma_poa = ".$_GET['subprg']." AND `obras`.`status_obra` = '4' AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
...
INNER JOIN sectores ON dependencias.id_sector = sectores.id_sector
INNER JOIN poa02_origen ON obras.id_obra = poa02_origen.id_obra
	  			   WHERE obras.subprograma_poa = ".$_GET['subprg']." AND `obras`.`status_obra` = '4' AND `poa02_origen`.`status_of` = 2 AND `poa02_origen`.`id_oficio` != 0 
  		           AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
               OR   obras.subprograma_poa = ".$_GET['subprg']." AND `obras`.`status_obra` = '4' AND `poa02_origen`.`status_of` = 2 AND `poa02_origen`.`id_oficio` != 0 
  		           AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999	
               group by obras.id_obra	   
	  			  ORDER BY `dependencias`.`id_dependencia` ASC, `proyectos`.`no_proyecto` ASC, `obras`.`id_obra` ASC";	  }
...
INNER JOIN sectores ON dependencias.id_sector = sectores.id_sector
INNER JOIN poa02_origen ON obras.id_obra = poa02_origen.id_obra
	  			   WHERE obras.programa_poa = ".$_GET['prg']." AND `obras`.`status_obra` = '4' AND `poa02_origen`.`status_of` = 2 AND `poa02_origen`.`id_oficio` != 0 
  		           AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
...
INNER JOIN subprogramas_poa ON obras.subprograma_poa = subprogramas_poa.id_subprograma_poa
INNER JOIN sectores ON dependencias.id_sector = sectores.id_sector
	  	 WHERE obras.subprograma_poa = ".$_GET['subprg']." AND obras.status_obra >= 3
	  	 ORDER BY dependencias.id_dependencia ASC, proyectos.no_proyecto ASC, obras.id_obra ASC";
	 	  		}	
...
INNER JOIN subprogramas_poa ON obras.subprograma_poa = subprogramas_poa.id_subprograma_poa
INNER JOIN sectores ON dependencias.id_sector = sectores.id_sector
	  	 WHERE  obras.programa_poa = ".$_GET['prg']." AND obras.status_obra >= 3
	  	 ORDER BY dependencias.id_dependencia ASC, proyectos.no_proyecto ASC, obras.id_obra ASC";
	 	  		}	  		  		
...
//$pdf->Text(11,198,'Número de Obras: '.$nn);
	...
	$pdf->Output('Programa-Subprograma.pdf','I');
//cerrar conexion */
