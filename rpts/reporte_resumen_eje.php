...
$id_dependencia = $_SESSION['id_dependencia_v3'];
	  ...
	  $query="SELECT * from eje order by id_eje asc ";
...
$ya=1;
	...
	if (mysql_num_rows($consulta_obras)>=1){
	 ...
	 $pdf->SetX(3);
	 $pdf->MultiCell(210,5,"NO. EJE: ".substr($row1['eje'],0,1)."  EJE: ".ucwords(strtolower(limpiar(substr($row1['eje'],2)))),0,L,0);
...
$pdf->Row(array("DEPENDENCIA","NO. OBRAS","APORTE FEDERAL","APORTE ESTATAL","APORTE MUNICIPAL","PARTICIPANTES","CREDITOS","OTROS","TOTAL"));
	 ...
	 $consulta_obras1 = mysql_query("SELECT dependencias.acronimo, Sum(obras.federal) AS SumaDefederal, Sum(obras.estatal) AS SumaDeestatal, Sum(obras.municipal) AS SumaDemunicipal, Sum(obras.participantes) AS SumaDeparticipantes, Sum(obras.credito) AS SumaDecredito, Sum(obras.otros) AS SumaDeotros, Sum(obras.federal+obras.estatal+obras.municipal+obras.participantes+obras.credito+obras.otros) AS total, Count(dependencias.acronimo) AS CuentaDeacronimo , eje.id_eje, eje.eje
FROM
obras
...
INNER JOIN sectores ON dependencias.id_sector = sectores.id_sector
 where obras.status_obra>='3' and eje.id_eje=".$row1['id_eje']."
...
$pdf->SetAligns(array(C,C,R,R,R,R,R,R,R));
	...
	$pdf->Row(array("Total Por Eje:",$cp,"$ ".number_format($fd,2),"$ ".number_format($st,2),"$ ".number_format($mn,2),"$ ".number_format($pt,2),"$ ".number_format($cr,2),"$ ".number_format($ot,2),"$ ".number_format($tt,2)));
...
$id_dependencia = $_SESSION['id_dependencia_v3'];
	  ...
	  $query="SELECT `proyectos`.`id_eje`, `obras`.`id_dependencia`, `poa02_origen`.`id_obra`, `poa02_origen`.`id_oficio`, `poa02_origen`.`status_of`, `eje`.`id_eje`, `eje`.`eje` 
	  FROM `obras` AS `obras`, `poa02_origen` AS `poa02_origen`, `proyectos` AS `proyectos`, `eje` AS `eje`
	   WHERE `obras`.`id_obra` = `poa02_origen`.`id_obra` AND `proyectos`.`id_proyecto` = `obras`.`id_proyecto` AND `eje`.`id_eje` = `proyectos`.`id_eje` 
	   AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
	AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
OR  `obras`.`id_obra` = `poa02_origen`.`id_obra` AND `proyectos`.`id_proyecto` = `obras`.`id_proyecto` AND `eje`.`id_eje` = `proyectos`.`id_eje` 
	   AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
	AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999
	   GROUP BY `proyectos`.`id_eje` ORDER BY `proyectos`.`id_eje` ASC ";
$consulta_obras = mysql_query($query,$siplan_data_conn) or die (mysql_error());
...
$pdf->Row(array("DEPENDENCIA","NO. OBRAS","APORTE FEDERAL","APORTE ESTATAL","APORTE MUNICIPAL","PARTICIPANTES","CREDITOS","OTROS","TOTAL"));
	 ...
	 $consulta_obras1 = mysql_query("SELECT proyectos.id_eje, obras.id_dependencia, poa02_origen.id_obra, poa02_origen.id_oficio, poa02_origen.status_of, dependencias.id_dependencia, dependencias.acronimo
	 FROM obras AS obras, poa02_origen AS poa02_origen, proyectos AS proyectos, dependencias AS dependencias
	  WHERE obras.id_obra = poa02_origen.id_obra AND proyectos.id_proyecto = obras.id_proyecto AND obras.id_dependencia = dependencias.id_dependencia
	  AND poa02_origen.id_oficio != 0 AND poa02_origen.status_of = 2 
	 AND poa02_origen.s07c_partid BETWEEN 4000 AND 4999 and proyectos.id_eje=".$row1['id_eje']."
OR  obras.id_obra = poa02_origen.id_obra AND proyectos.id_proyecto = obras.id_proyecto AND obras.id_dependencia = dependencias.id_dependencia
	  AND poa02_origen.id_oficio != 0 AND poa02_origen.status_of = 2 
AND poa02_origen.s07c_partid BETWEEN 6000 AND 6999 and proyectos.id_eje=".$row1['id_eje']."
	 GROUP BY obras.id_dependencia ORDER BY obras.id_dependencia ASC" ,$siplan_data_conn) or die (mysql_error());

/*SELECT dependencias.id_dependencia, dependencias.acronimo, eje.id_eje, Count(dependencias.acronimo) AS CuentaDeacronimo
FROM (dependencias INNER JOIN (obras INNER JOIN aportes ON obras.id_obra = aportes.id_obra) ON dependencias.id_dependencia = obras.id_dependencia) INNER JOIN (eje INNER JOIN proyectos ON eje.id_eje = proyectos.id_eje) ON (proyectos.id_proyecto = obras.id_proyecto) AND (dependencias.id_dependencia = proyectos.id_dependencia) where obras.status_obra='4'  and eje.id_eje=".$row1['id_eje']."
...
//QUITE GROUP BY `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `proyectos`.`id_dependencia`

$sac_aportes_eje = mysql_query("SELECT `proyectos`.`id_dependencia`, `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `proyectos`.`id_dependencia`, SUM( `poa02_origen`.`monto` ) AS `sumamonto`, `proyectos`.`id_eje` 
FROM `proyectos` AS `proyectos`, `poa02_origen` AS `poa02_origen` 
WHERE `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `poa02_origen`.`tipo` = 0  AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `proyectos`.`id_eje` = ".$row['id_eje']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND poa02_origen.s07c_partid BETWEEN 4000 AND 4999
OR  `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `poa02_origen`.`tipo` = 0  AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `proyectos`.`id_eje` = ".$row['id_eje']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
...
");
	...
	//sacar tipo ampliaciones buscar por dependecia y subprograma y mas el between
 	$sac_aportes_amp = mysql_query("SELECT `proyectos`.`id_dependencia`, `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `proyectos`.`id_dependencia`, SUM( `poa02_origen`.`monto` ) AS `sumamonto`, `proyectos`.`id_eje` 
FROM `proyectos` AS `proyectos`, `poa02_origen` AS `poa02_origen` 
WHERE `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `poa02_origen`.`tipo` = 1  AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `proyectos`.`id_eje` = ".$row['id_eje']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND poa02_origen.s07c_partid BETWEEN 4000 AND 4999
OR `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `poa02_origen`.`tipo` = 1  AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `proyectos`.`id_eje` = ".$row['id_eje']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND poa02_origen.s07c_partid BETWEEN 6000 AND 6999 GROUP BY poa02_origen.s08c_origen
");
	...
	//sacar tipo cancelaciones buscar por dependecia y subprograma y mas el between
 	$sac_aportes_can = mysql_query("SELECT `proyectos`.`id_dependencia`, `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `proyectos`.`id_dependencia`, SUM( `poa02_origen`.`monto` ) AS `sumamonto`, `proyectos`.`id_eje` 
FROM `proyectos` AS `proyectos`, `poa02_origen` AS `poa02_origen` 
WHERE `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `poa02_origen`.`tipo` = 2 AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `proyectos`.`id_eje` = ".$row['id_eje']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND poa02_origen.s07c_partid BETWEEN 4000 AND 4999
OR `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `poa02_origen`.`tipo` = 2 AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `proyectos`.`id_eje` = ".$row['id_eje']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND poa02_origen.s07c_partid BETWEEN 6000 AND 6999 GROUP BY poa02_origen.s08c_origen
");
...
$totals=$federal+$estatal+$municipal+$particip+$credito+$otros;
				...
				$sac_cuenta = mysql_query("SELECT `proyectos`.`id_eje`, `obras`.`id_dependencia`, `poa02_origen`.`id_obra`, `poa02_origen`.`id_oficio`, `poa02_origen`.`status_of` 
FROM `obras` AS `obras`, `poa02_origen` AS `poa02_origen`, `proyectos` AS `proyectos`
 WHERE `obras`.`id_obra` = `poa02_origen`.`id_obra` AND `proyectos`.`id_proyecto` = `obras`.`id_proyecto` AND 
 `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
 and obras.id_dependencia=".$row['id_dependencia']." and proyectos.id_eje=".$row['id_eje']."
AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999 
OR   `obras`.`id_obra` = `poa02_origen`.`id_obra` AND `proyectos`.`id_proyecto` = `obras`.`id_proyecto` AND 
 `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
 and obras.id_dependencia=".$row['id_dependencia']." and proyectos.id_eje=".$row['id_eje']."
AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999
GROUP BY `poa02_origen`.`id_obra`
ORDER BY `proyectos`.`id_eje` ASC	");
