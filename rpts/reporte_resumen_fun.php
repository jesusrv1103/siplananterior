...
$id_dependencia = $_SESSION['id_dependencia_v3'];
	  ...
	  $query="SELECT * from funcion order by id_funcion asc ";
...
$ya=1;
	...
	if (mysql_num_rows($consulta_obras)>=1){
	 ...
	 $pdf->SetX(3);
	 $pdf->MultiCell(210,5,"NO. FUNCION: ".($row1['id_funf'])."  FUNCION: ".ucwords(strtolower(limpiar((utf8_decode($row1['nombre']))))),0,L,0);
...
where  obras.status_obra>='3' and proyectos.funcion=".$row1['id_funf']." and  proyectos.finalidad=".$row1['id_finalidad']."
...
$id_dependencia = $_SESSION['id_dependencia_v3'];
	  ...
	  $query="SELECT `funcion`.`id_funcion`, `funcion`.`nombre`, `funcion`.`descripcion`
...
$ya=1;
	...
	if (mysql_num_rows($consulta_obras)>=1){
     ...
     $pdf->SetAligns(array(C,C,C,C,C));
	 $pdf->SetX(3);
	 $pdf->MultiCell(210,5,"NO. FUNCION: ".($row1['id_funcion'])."  FUNCION: ".ucwords(strtolower(limpiar((utf8_decode($row1['nombre']))))),0,L,0);
	 
	  $pdf->SetX(3);
	// $pdf->Row(array("No. Oficio","Tipo","No. Obra","Importe X Obra","Total X Oficio"));
	 	$pdf->SetAligns(array(C,C,Ch,C,C,C,C,C,C));
$pdf->Row(array("DEPENDENCIA","NO. OBRAS","APORTE FEDERAL","APORTE ESTATAL","APORTE MUNICIPAL","PARTICIPANTES","CREDITOS","OTROS","TOTAL"));
	 }

	 
	 $consulta_obras1 = mysql_query("SELECT `dependencias`.`id_dependencia`, `dependencias`.`acronimo`, `funcion`.`id_funcion`
	  FROM `obras` AS `obras`, `proyectos` AS `proyectos`, `funcion` AS `funcion`, `poa02_origen` AS `poa02_origen`, `dependencias` AS `dependencias` 
	  WHERE `obras`.`id_proyecto` = `proyectos`.`id_proyecto` AND `funcion`.`id_funcion` = `proyectos`.`funcion` AND `poa02_origen`.`id_obra` = `obras`.`id_obra` AND `dependencias`.`id_dependencia` = `obras`.`id_dependencia` 
	 AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
	  		AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
			AND `funcion`.`id_funcion`=".$row1['id_funcion']." 
			OR `obras`.`id_proyecto` = `proyectos`.`id_proyecto` AND `funcion`.`id_funcion` = `proyectos`.`funcion` AND `poa02_origen`.`id_obra` = `obras`.`id_obra` AND `dependencias`.`id_dependencia` = `obras`.`id_dependencia` 
	 AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
			AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999
			AND `funcion`.`id_funcion`=".$row1['id_funcion']." 
...
$sac_aportes_eje = mysql_query("SELECT `proyectos`.`id_dependencia`, `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `proyectos`.`id_dependencia`, SUM( `poa02_origen`.`monto` ) AS `sumamonto`, `proyectos`.`id_eje`, `funcion`.`id_funcion` 
FROM `proyectos` AS `proyectos`, `poa02_origen` AS `poa02_origen`, `funcion` AS `funcion` 
WHERE `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `funcion`.`id_funcion` = `proyectos`.`funcion` 
AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `poa02_origen`.`tipo` = 0 AND `funcion`.`id_funcion` = ".$row['id_funcion']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
OR `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `funcion`.`id_funcion` = `proyectos`.`funcion` 
AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `poa02_origen`.`tipo` = 0 AND `funcion`.`id_funcion` = ".$row['id_funcion']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999
");
	...
	//sacar tipo ampliaciones buscar por dependecia y subprograma y mas el between
 	$sac_aportes_amp = mysql_query("SELECT `proyectos`.`id_dependencia`, `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `proyectos`.`id_dependencia`, SUM( `poa02_origen`.`monto` ) AS `sumamonto`, `proyectos`.`id_eje`, `funcion`.`id_funcion` 
FROM `proyectos` AS `proyectos`, `poa02_origen` AS `poa02_origen`, `funcion` AS `funcion` 
WHERE `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `funcion`.`id_funcion` = `proyectos`.`funcion` 
AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `poa02_origen`.`tipo` = 1 AND `funcion`.`id_funcion` = ".$row['id_funcion']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
OR `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `funcion`.`id_funcion` = `proyectos`.`funcion` 
AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `poa02_origen`.`tipo` = 1 AND `funcion`.`id_funcion` = ".$row['id_funcion']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999");
	...
	//sacar tipo cancelaciones buscar por dependecia y subprograma y mas el between
 	$sac_aportes_can = mysql_query("SELECT `proyectos`.`id_dependencia`, `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `proyectos`.`id_dependencia`, SUM( `poa02_origen`.`monto` ) AS `sumamonto`, `proyectos`.`id_eje`, `funcion`.`id_funcion` 
FROM `proyectos` AS `proyectos`, `poa02_origen` AS `poa02_origen`, `funcion` AS `funcion` 
WHERE `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `funcion`.`id_funcion` = `proyectos`.`funcion` 
AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `poa02_origen`.`tipo` = 2 AND `funcion`.`id_funcion` = ".$row['id_funcion']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
OR `proyectos`.`id_proyecto` = `poa02_origen`.`id_proyecto` AND `funcion`.`id_funcion` = `proyectos`.`funcion` 
AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `poa02_origen`.`tipo` = 2 AND `funcion`.`id_funcion` = ".$row['id_funcion']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999
...
SELECT `dependencias`.`id_dependencia`, `dependencias`.`acronimo`, `funcion`.`id_funcion`
	  FROM `obras` AS `obras`, `proyectos` AS `proyectos`, `funcion` AS `funcion`, `poa02_origen` AS `poa02_origen`, `dependencias` AS `dependencias` 
	  WHERE `obras`.`id_proyecto` = `proyectos`.`id_proyecto` AND `funcion`.`id_funcion` = `proyectos`.`funcion` AND `poa02_origen`.`id_obra` = `obras`.`id_obra` AND `dependencias`.`id_dependencia` = `obras`.`id_dependencia` 
	 AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
			AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
			AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `funcion`.`id_funcion` = ".$row['id_funcion']."
			OR `obras`.`id_proyecto` = `proyectos`.`id_proyecto` AND `funcion`.`id_funcion` = `proyectos`.`funcion` AND `poa02_origen`.`id_obra` = `obras`.`id_obra` AND `dependencias`.`id_dependencia` = `obras`.`id_dependencia` 
	 AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
			AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999
			AND `proyectos`.`id_dependencia` = '".$row['id_dependencia']."' AND `funcion`.`id_funcion` = ".$row['id_funcion']."
