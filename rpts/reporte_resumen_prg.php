...
SELECT dependencias.acronimo, Sum(obras.federal) AS SumaDefederal, Sum(obras.estatal) AS SumaDeestatal, Sum(obras.municipal) AS SumaDemunicipal, Sum(obras.participantes) AS SumaDeparticipantes, Sum(obras.credito) AS SumaDecredito, Sum(obras.otros) AS SumaDeotros, Sum(obras.federal+obras.estatal+obras.municipal+obras.participantes+obras.credito+obras.otros) AS total, Count(dependencias.acronimo) AS CuentaDeacronimo,subprogramas_poa.id_subprograma_poa, subprogramas_poa.descripcion
FROM
obras
...
WHERE  `subprogramas_poa`.`id_subprograma_poa` = ".$row2['id_subprograma_poa']." and `obras`.`status_obra`>=3 GROUP BY `dependencias`.`acronimo`, `subprogramas_poa`.`id_subprograma_poa`,`subprogramas_poa`.`descripcion`" ,$siplan_data_conn) or die (mysql_error());
	$pdf->SetFont('Arial','',9);
...
$id_dependencia = $_SESSION['id_dependencia_v3'];
	  ...
	  GROUP BY `programas_poa`.`id_programa_poa` ORDER BY `programas_poa`.`clave` ASC";
$consulta_obras = mysql_query($query,$siplan_data_conn) or die (mysql_error());
