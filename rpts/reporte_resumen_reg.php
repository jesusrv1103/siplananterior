...
$id_dependencia = $_SESSION['id_dependencia_v3'];
	  ...
	  $query="SELECT * from regiones order by id_region asc ";
...
$ya=1;
	...
	if (mysql_num_rows($consulta_obras)>=1){
	 ...
	 $pdf->SetX(3);
	 $pdf->MultiCell(210,5,"NO. REGION: ".$row1['id_region']."  REGION: ".ucwords(utf8_decode(strtolower(limpiar($row1['nombre'])))),0,L,0);
...
$pdf->Row(array("DEPENDENCIA","NO. OBRAS","APORTE FEDERAL","APORTE ESTATAL","APORTE MUNICIPAL","PARTICIPANTES","CREDITOS","OTROS","TOTAL"));
	 ...
	 $consulta_obras1 = mysql_query("SELECT dependencias.acronimo, Sum(obras.federal) AS SumaDefederal, Sum(obras.estatal) AS SumaDeestatal, Sum(obras.municipal) AS SumaDemunicipal, Sum(obras.participantes) AS SumaDeparticipantes, Sum(obras.credito) AS SumaDecredito, Sum(obras.otros) AS SumaDeotros, Sum(obras.federal+obras.estatal+obras.municipal+obras.participantes+obras.credito+obras.otros) AS total, Count(dependencias.acronimo) AS CuentaDeacronimo,regiones.nombre, regiones.id_region
FROM
obras
...
where regiones.id_region=".$row1['id_region']." and obras.status_obra>=3
...
$id_dependencia = $_SESSION['id_dependencia_v3'];
	  ...
	  $query="SELECT `regiones`.`id_region`, `regiones`.`nombre` 
	  FROM `poa02_origen` AS `poa02_origen`, `obras` AS `obras`, `regiones` AS `regiones`, `municipios` AS `municipios` 
	  WHERE `poa02_origen`.`id_obra` = `obras`.`id_obra` AND `regiones`.`id_region` = `municipios`.`id_region` AND `obras`.`municipio` = `municipios`.`id_municipio`
	   AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
	   AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
 		OR `poa02_origen`.`id_obra` = `obras`.`id_obra` AND `regiones`.`id_region` = `municipios`.`id_region` AND `obras`.`municipio` = `municipios`.`id_municipio`
	   AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
	   AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999
	  GROUP BY `regiones`.`id_region` ORDER BY `regiones`.`id_region` ASC";
$consulta_obras = mysql_query($query,$siplan_data_conn) or die (mysql_error());
...
$pdf->Row(array("DEPENDENCIA","NO. OBRAS","APORTE FEDERAL","APORTE ESTATAL","APORTE MUNICIPAL","PARTICIPANTES","CREDITOS","OTROS","TOTAL"));
	 ...
	 $consulta_obras1 = mysql_query("SELECT `dependencias`.`acronimo`, `regiones`.`id_region`, `poa02_origen`.`id_oficio`, `poa02_origen`.`status_of` 
	 FROM `dependencias` AS `dependencias`, `obras` AS `obras`, `municipios` AS `municipios`, `regiones` AS `regiones`, `poa02_origen` AS `poa02_origen` 
	 WHERE `dependencias`.`id_dependencia` = `obras`.`id_dependencia` AND `municipios`.`id_municipio` = `obras`.`municipio` AND `regiones`.`id_region` = `municipios`.`id_region` AND `obras`.`id_obra` = `poa02_origen`.`id_obra` 
	 AND `regiones`.`id_region` = ".$row1['id_region']." AND `obras`.`status_obra` = '4' AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2
	 AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
 		OR `dependencias`.`id_dependencia` = `obras`.`id_dependencia` AND `municipios`.`id_municipio` = `obras`.`municipio` AND `regiones`.`id_region` = `municipios`.`id_region` AND `obras`.`id_obra` = `poa02_origen`.`id_obra` 
	 AND `regiones`.`id_region` = ".$row1['id_region']." AND `obras`.`status_obra` = '4' AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2
	AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999
	 GROUP BY `dependencias`.`acronimo`",$siplan_data_conn) or die (mysql_error());
	...
	//quitando nuevamente GROUP BY `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `dependencias`.`acronimo`
	
$sac_aportes_eje = mysql_query("SELECT `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `dependencias`.`acronimo`, SUM( `poa02_origen`.`monto` ) AS `sumamonto`, `regiones`.`id_region`, `regiones`.`nombre` 
FROM `obras` AS `obras`, `poa02_origen` AS `poa02_origen`, `dependencias` AS `dependencias`, `regiones` AS `regiones`, `municipios` AS `municipios`
 WHERE `obras`.`id_obra` = `poa02_origen`.`id_obra` AND `dependencias`.`id_dependencia` = `obras`.`id_dependencia` AND `regiones`.`id_region` = `municipios`.`id_region` AND `municipios`.`id_municipio` = `obras`.`municipio` 
 AND `poa02_origen`.`tipo` = 0 AND `dependencias`.`acronimo` = '".$row['acronimo']."' AND `regiones`.`id_region` = ".$row['id_region']." 
 AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
 AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
 OR `obras`.`id_obra` = `poa02_origen`.`id_obra` AND `dependencias`.`id_dependencia` = `obras`.`id_dependencia` AND `regiones`.`id_region` = `municipios`.`id_region` AND `municipios`.`id_municipio` = `obras`.`municipio` 
 AND `poa02_origen`.`tipo` = 0 AND `dependencias`.`acronimo` = '".$row['acronimo']."' AND `regiones`.`id_region` = ".$row['id_region']." 
 AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
 ...
 ");
	...
	//sacar tipo ampliaciones buscar por dependecia y subprograma y mas el between
 	$sac_aportes_amp = mysql_query("SELECT `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `dependencias`.`acronimo`, SUM( `poa02_origen`.`monto` ) AS `sumamonto`, `regiones`.`id_region`, `regiones`.`nombre` 
FROM `obras` AS `obras`, `poa02_origen` AS `poa02_origen`, `dependencias` AS `dependencias`, `regiones` AS `regiones`, `municipios` AS `municipios`
 WHERE `obras`.`id_obra` = `poa02_origen`.`id_obra` AND `dependencias`.`id_dependencia` = `obras`.`id_dependencia` AND `regiones`.`id_region` = `municipios`.`id_region` AND `municipios`.`id_municipio` = `obras`.`municipio` AND `poa02_origen`.`tipo` = 1 AND `dependencias`.`acronimo` = '".$row['acronimo']."' AND `regiones`.`id_region` = ".$row['id_region']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
...
AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999 GROUP BY poa02_origen.s08c_origen
 ");
	...
	//sacar tipo cancelaciones buscar por dependecia y subprograma y mas el between
 	$sac_aportes_can = mysql_query("SELECT `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `dependencias`.`acronimo`, SUM( `poa02_origen`.`monto` ) AS `sumamonto`, `regiones`.`id_region`, `regiones`.`nombre` 
FROM `obras` AS `obras`, `poa02_origen` AS `poa02_origen`, `dependencias` AS `dependencias`, `regiones` AS `regiones`, `municipios` AS `municipios`
 WHERE `obras`.`id_obra` = `poa02_origen`.`id_obra` AND `dependencias`.`id_dependencia` = `obras`.`id_dependencia` AND `regiones`.`id_region` = `municipios`.`id_region` AND `municipios`.`id_municipio` = `obras`.`municipio` AND `poa02_origen`.`tipo` = 2 AND `dependencias`.`acronimo` = '".$row['acronimo']."' AND `regiones`.`id_region` = ".$row['id_region']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
 OR `obras`.`id_obra` = `poa02_origen`.`id_obra` AND `dependencias`.`id_dependencia` = `obras`.`id_dependencia` AND `regiones`.`id_region` = `municipios`.`id_region` AND `municipios`.`id_municipio` = `obras`.`municipio` AND `poa02_origen`.`tipo` = 2 AND `dependencias`.`acronimo` = '".$row['acronimo']."' AND `regiones`.`id_region` = ".$row['id_region']." AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2 
AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999 GROUP BY poa02_origen.s08c_origen
");
...
$totals=$federal+$estatal+$municipal+$particip+$credito+$otros;
		...
		$sac_cuenta = mysql_query("SELECT `dependencias`.`acronimo`, ( `dependencias`.`acronimo` ) AS `CuentaDeacronimo`, `regiones`.`id_region`, `poa02_origen`.`id_oficio`, `poa02_origen`.`status_of` 
	 FROM `dependencias` AS `dependencias`, `obras` AS `obras`, `municipios` AS `municipios`, `regiones` AS `regiones`, `poa02_origen` AS `poa02_origen` 
	 WHERE `dependencias`.`id_dependencia` = `obras`.`id_dependencia` AND `municipios`.`id_municipio` = `obras`.`municipio` AND `regiones`.`id_region` = `municipios`.`id_region` AND `obras`.`id_obra` = `poa02_origen`.`id_obra` 
	 AND `regiones`.`id_region` = ".$row['id_region']." AND dependencias.acronimo='".$row['acronimo']."' AND `obras`.`status_obra` = '4' AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2
	 AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
 		OR `dependencias`.`id_dependencia` = `obras`.`id_dependencia` AND `municipios`.`id_municipio` = `obras`.`municipio` AND `regiones`.`id_region` = `municipios`.`id_region` AND `obras`.`id_obra` = `poa02_origen`.`id_obra` 
	 AND `regiones`.`id_region` = ".$row['id_region']." AND dependencias.acronimo='".$row['acronimo']."' AND `obras`.`status_obra` = '4' AND `poa02_origen`.`id_oficio` != 0 AND `poa02_origen`.`status_of` = 2
	 AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999
