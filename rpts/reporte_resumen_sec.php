...
$ya=1;
	...
	if (mysql_num_rows($consulta_obras)>=1){
	 ...
	 $pdf->SetX(3);
	 $pdf->MultiCell(210,5,"NO. SECTOR: ".$row1['id_sector']."  SECTOR: ".ucwords(strtolower(limpiar(utf8_decode($row1['sector'])))),0,L,0);
...
$pdf->Row(array("DEPENDENCIA","NO. OBRAS","APORTE FEDERAL","APORTE ESTATAL","APORTE MUNICIPAL","PARTICIPANTES","CREDITOS","OTROS","TOTAL"));
	 ...
	 SELECT dependencias.acronimo, Sum(obras.federal) AS SumaDefederal, Sum(obras.estatal) AS SumaDeestatal, Sum(obras.municipal) AS SumaDemunicipal, Sum(obras.participantes) AS SumaDeparticipantes, Sum(obras.credito) AS SumaDecredito, Sum(obras.otros) AS SumaDeotros, Sum(obras.federal+obras.estatal+obras.municipal+obras.participantes+obras.credito+obras.otros) AS total, Count(dependencias.acronimo) AS CuentaDeacronimo, sectores.id_sector, sectores.sector
FROM
obras
...
where sectores.id_sector=".$row1['id_sector']." and obras.status_obra>=3
...
$pdf->Row(array("DEPENDENCIA","NO. OBRAS","APORTE FEDERAL","APORTE ESTATAL","APORTE MUNICIPAL","PARTICIPANTES","CREDITOS","OTROS","TOTAL"));
	 ...
	 SELECT dependencias.id_dependencia, dependencias.acronimo, Count(dependencias.acronimo) AS CuentaDeacronimo, sectores.id_sector, sectores.sector
FROM
obras
INNER JOIN dependencias ON obras.id_dependencia = dependencias.id_dependencia
INNER JOIN proyectos ON obras.id_proyecto = proyectos.id_proyecto
...
where obras.status_obra='4' and sectores.id_sector=".$row1['id_sector']."
 GROUP BY dependencias.acronimo, sectores.id_sector, sectores.sector" ,$siplan_data_conn) or die (mysql_error());

	$pdf->SetFont('Arial','',9);
	while($row = mysql_fetch_array($consulta_obras1))

	{ //qute GROUP BY `poa02_origen`.`s08c_origen`, `poa02_origen`.`tipo`, `dependencias`.`acronimo`
...
AND `dependencias`.`acronimo` = '".$row['acronimo']."' and dependencias.id_sector=".$row['id_sector']." and poa02_origen.id_oficio!=0 and poa02_origen.status_of=2  
...
SELECT `dependencias`.`acronimo`, COUNT( dependencias.acronimo ) AS `CuentaDeacronimo`, `sectores`.`id_sector`, `sectores`.`sector` 
FROM `dependencias` AS `dependencias`, `obras` AS `obras`, `sectores` AS `sectores`, `poa02_origen` AS `poa02_origen`
 WHERE `dependencias`.`id_dependencia` = `obras`.`id_dependencia` AND `sectores`.`id_sector` = `dependencias`.`id_sector` 
AND `obras`.`id_obra` = `poa02_origen`.`id_obra` AND `obras`.`status_obra` = '4'AND poa02_origen.status_of =2
and poa02_origen.id_oficio!=0 
AND obras.id_dependencia=".$row['id_dependencia']."
AND dependencias.id_sector=".$row['id_sector']."
AND `poa02_origen`.`s07c_partid` BETWEEN 4000 AND 4999
OR `dependencias`.`id_dependencia` = `obras`.`id_dependencia` AND `sectores`.`id_sector` = `dependencias`.`id_sector` 
AND `obras`.`id_obra` = `poa02_origen`.`id_obra` AND `obras`.`status_obra` = '4'AND poa02_origen.status_of =2
and poa02_origen.id_oficio!=0 
AND obras.id_dependencia=".$row['id_dependencia']."
AND dependencias.id_sector=".$row['id_sector']."
AND `poa02_origen`.`s07c_partid` BETWEEN 6000 AND 6999 
GROUP BY poa02_origen.id_obra,`dependencias`.`acronimo`, `sectores`.`sector`, `sectores`.`id_sector`");
