...
header("Content-Disposition: attachment; filename=Avances_de_Ejercicio_Físico_Financiero_201.xls");
...
<table border=0 cellpadding=0 cellspacing=0 width=3422 style='border-collapse:
 ...
 <tr height=30 style='mso-height-source:userset;height:22.5pt'>
  ...
  height:22.5pt'>Avance de Ejercicio F&iacute;sico - Financiero 201</td>
 </tr>
...
// SELECT * FROM localidades WHERE id_localidad = ".$row['localidad']
 ...
 $sumar_aportes = mysql_query("select  (federal + estatal + municipal + participantes + credito + otros ) as totalp  from obras where id_obra = ".$row['id_obra'],$siplan_data_conn) or die (mysql_error());
 ...
 $sac_aportes = mysql_query("select  * from obras where id_obra = ".$row['id_obra'],$siplan_data_conn) or die (mysql_error());
...
$res_consulta_act= mysql_query("select * from acciones where id_accion=".$row['id_actividad']);
	...
	$PED=substr($dat_consulta_str['nombre'],0,6);
 //PED
...
$st_edo="Cancelada";	
 ...
 <tr class=xl144 height=17 style='height:12.75pt'>
  ...
  <td class=xl201 width=90 style='border-left:none;width:68pt'><?php echo $ampli; ?></td>
  <td class=xl201 width=90 style='border-left:none;width:68pt'><?php echo $cance; ?></td>
