...
header("Content-Disposition: attachment; filename=Reportes -Aprobado PP2018.xls");
...
<table border=0 cellpadding=0 cellspacing=0 width=3422 style='border-collapse:
 ...
 <tr height=30 style='mso-height-source:userset;height:22.5pt'>
  ...
  height:22.5pt'>COORDINACIÓN ESTATAL DE PLANEACI&Oacute;N</td>
 </tr>
 <tr height=30 style='mso-height-source:userset;height:22.5pt'>
  <td colspan=40 height=30 class=xl138 style='border-right:.5pt solid black;
  height:22.5pt'>Avance de Ejercicio F&iacute;sico - Financiero 2018</td>
 </tr>
 ...
 <tr class=xl110 height=38 style='mso-height-source:userset;height:28.5pt'>
  ...
  <?php  	if ($_POST['rpt']=="Capitulos") { ?>

   <td rowspan=3 class=xl109 width=98 style='border-top:none;width:74pt'>CAPITULO</td>
 ...
 <tr class=xl110 height=47 style='mso-height-source:userset;height:35.25pt'>
  ...
  <td rowspan=2 class=xl215 width=80 style='border-bottom:1.0pt solid black;  border-top:none;width:60pt'>F&Iacute;SICO</td>
    <td rowspan=2 class=xl109 width=206 style='border-top:none;width:65pt'>&Uacute;LTIMA ACTUALIZACI&Oacute;N</td>
...
$query.=" and dependencias.id_dependencia=".$dep;
	...
	// echo "SELECT *,(obras.cantidad) as cant_pro, (proyectos.nombre) as nom_proy FROM oficio_aprobacion INNER JOIN ((sectores INNER JOIN (regiones INNER JOIN (municipios INNER JOIN ((dependencias INNER JOIN (eje INNER JOIN proyectos ON eje.id_eje = proyectos.id_eje) ON dependencias.id_dependencia = proyectos.id_dependencia) INNER JOIN obras ON (proyectos.id_proyecto = obras.id_proyecto) AND (dependencias.id_dependencia = obras.id_dependencia)) ON municipios.id_municipio = obras.municipio) ON regiones.id_region = municipios.id_region) ON sectores.id_sector = dependencias.id_sector) INNER JOIN detalle_oficio ON (proyectos.id_proyecto = detalle_oficio.id_proyecto) AND (obras.id_obra = detalle_oficio.id_poa02)) ON oficio_aprobacion.id_oficio = detalle_oficio.id_oficio WHERE obras.status_obra ='4' AND oficio_aprobacion.tipo=0 AND oficio_aprobacion.no_oficio !='' ".$query." GROUP BY detalle_oficio.id_detalle_of ORDER BY dependencias.id_dependencia, proyectos.no_proyecto, obras.obra";
		      
	  $consulta_obras = mysql_query("SELECT *,(obras.cantidad) as cant_pro, (proyectos.nombre) as nom_proy FROM oficio_aprobacion INNER JOIN ((sectores INNER JOIN (regiones INNER JOIN (municipios INNER JOIN ((dependencias INNER JOIN (eje INNER JOIN proyectos ON eje.id_eje = proyectos.id_eje) ON dependencias.id_dependencia = proyectos.id_dependencia) INNER JOIN obras ON (proyectos.id_proyecto = obras.id_proyecto) AND (dependencias.id_dependencia = obras.id_dependencia)) ON municipios.id_municipio = obras.municipio) ON regiones.id_region = municipios.id_region) ON sectores.id_sector = dependencias.id_sector) INNER JOIN detalle_oficio ON (proyectos.id_proyecto = detalle_oficio.id_proyecto) AND (obras.id_obra = detalle_oficio.id_poa02)) ON oficio_aprobacion.id_oficio = detalle_oficio.id_oficio WHERE obras.status_obra ='4' AND oficio_aprobacion.activo=1 AND oficio_aprobacion.tipo=0 AND oficio_aprobacion.no_oficio !='' ".$query." GROUP BY detalle_oficio.id_detalle_of ORDER BY  obras.municipio * 1 ,  dependencias.id_dependencia, proyectos.no_proyecto, obras.obra ",$siplan_data_conn) or die (mysql_error()); 
...
//linea antigua  $sac_mun = mysql_query("SELECT * FROM municipios WHERE id_municipio = ".$row['municipio'],$siplan_data_conn) or die (mysql_error());
 ...
 $opds = array("61", "62", "63", "69", "71", "74", "75", "76","77","78");   
          if (in_array($res_dep['id_dependencia'], $opds)) {

 $consultar_fon = mysql_query("SELECT
origen".$res_dep['id_dependencia'].".c08c_tipori,
poa02_origen.id_obra
FROM
origen".$res_dep['id_dependencia']."
INNER JOIN poa02_origen ON origen".$res_dep['id_dependencia'].".s08c_origen = poa02_origen.s08c_origen
WHERE poa02_origen.id_obra = ".$row['id_obra']." group by origen".$res_dep['id_dependencia'].".s08c_origen asc",$siplan_data_conn) or die (mysql_error());
...
$partidad="";
	...
	$partidad1="";
	 ...
	 $consultar_partida = mysql_query("SELECT id_obra, s07c_partid

FROM

poa02_origen

WHERE id_obra = ".$row['id_obra']." group by s07c_partid asc",$siplan_data_conn) or die (mysql_error());

	

		  while ($rep_par = mysql_fetch_array($consultar_partida)) {

			  

			 $tpa= substr($rep_par['s07c_partid'],0,1);
...
FROM municipios INNER JOIN localidades ON municipios.id_municipio = localidades.id_municipio WHERE municipios.id_municipio=".$row['municipio']." and localidades.id_finanzas=".$row['localidad'],$siplan_data_conn) or die (mysql_error());
 ...
 $num_oficio="";
  ...
  $consultar_aproba = mysql_query("SELECT * FROM oficio_aprobacion  WHERE id_oficio = ".$rep_ofi['id_oficio']." AND oficio_aprobacion.activo=1",$siplan_data_conn) or die (mysql_error());
...
//	echo "select sum(monto) as sumamonto, s08c_origen from poa02_origen where id_poa02= ".$row['obra']." and id_oficio!=0 and status_of=2 and tipo=0 and s07c_partid BETWEEN 4000 AND 4999 
	...
	{ 
	  ...
	  $cance += $res_aporte_can['sumamonto'];
		 ...
		 if ($orige_n=="2103"){
                 $estatalc= $estatalc + $res_aporte_can['sumamonto'];

                } elseif($orige_n=="3102"){

			 $participc= $participc + $res_aporte_can['sumamonto'];
	...
	switch($orige_n)
	{
	  ...
	  case ($orige_n >=2000 && $orige_n <=2999):
	  $federalc= $federalc + $res_aporte_can['sumamonto'];
	  break;
	  
	  case ($orige_n >=3000 && $orige_n <=3999):
	  $municipalc= $municipalc + $res_aporte_can['sumamonto'];
	  break;
	  
	  default:
	  $otrosc = $otrosc + $res_aporte_can['sumamonto'];
	  break;  
	...
	$totalse=(($federal+$estatal+$municipal+$particip+$credito+$otros));
	$totalsc=(($federalc+$estatalc+$municipalc+$participc+$creditoc+$otrosc));
	$totals=(($totalse-$totalsc));
...
$st_edo="Cancelada";	
  ...
  $sac_ejercido_fec = mysql_query("select fecha from ejercido where activo=1 and id_obra = ".$row['id_obra']." order by fecha desc limit 1",$siplan_data_conn) or die (mysql_error());
  
  $st_ejercido_fec = mysql_fetch_array($sac_ejercido_fec);
 ...
 <?php  	if ($_POST['rpt']=="Capitulos") { ?>
    ...
    <td class=xl115 width=98 style='border-left:none;width:74pt'><?php echo $partidad; ?></td>
  ...
  <td class=xl66 width=90 style='border-left:none;width:68pt'><?php echo "$".number_format(($federal-$federalc),2); ?> </td>
  <td class=xl66 width=90 style='border-left:none;width:68pt'><?php echo "$".number_format(($estatal-$estatalc),2); ?> </td>
  <td class=xl66 width=90 style='border-left:none;width:68pt'><?php echo "$".number_format(($municipal-$municipalc),2); ?></td>
  <td class=xl66 width=90 style='border-left:none;width:68pt'><?php echo "$".number_format(($particip-$participc),2); ?></td>
  <td class=xl66 width=90 style='border-left:none;width:68pt'><?php echo "$".number_format(($credito-$creditoc),2); ?></td>
  <td class=xl66 width=90 style='border-left:none;width:68pt'><?php echo "$".number_format(($otros-$otrosc),2); ?></td>
  <td class=xl66 width=90 style='border-left:none;width:68pt'><?php echo "$".number_format($ampli,2); ?></td>
  <td class=xl66 width=90 style='border-left:none;width:68pt'><?php echo "$".number_format($cance,2); ?></td>
  ...
  <td class=xl143 width=80 style='border-left:none;width:60pt'><?php echo $fisico; ?></td>
   <td class=xl1411 width=80 style='border-left:none;width:60pt'><?php echo substr($st_ejercido_fec['fecha'],8,2)."-".substr($st_ejercido_fec['fecha'],5,2)."-".substr($st_ejercido_fec['fecha'],0,4); ?></td>
